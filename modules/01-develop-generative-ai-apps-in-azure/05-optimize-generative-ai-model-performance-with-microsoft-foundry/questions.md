...
A. Increase both `temperature` and `top_p` together for the brainstorming feature
...
C. Always set `temperature` to 1.0 for consistency across features
D. Use `top_p` for creative tasks and `temperature` for factual tasks simultaneously in the same request
...
A. Few-shot learning; required for all models including o-series
...
C. Persona pattern; required for all models including o-series
D. Format template pattern; irrelevant to reasoning quality
...
A. Remove all examples from the prompt
B. Increase the temperature to improve comprehension
...
A. Pick a temperature value, choose a model, set max tokens, add examples
...
C. List every possible user question in advance, then ban all others
D. Only specify the output format; the other elements are optional
...
A. A system message guarantees 100% compliant model behavior and requires no further testing
...
C. System messages are ignored by chat completion models
D. System messages can only be used with fine-tuned models
...
A. Few-shot learning; to provide more examples
B. Decomposing a task into explicit sub-steps across separate prompts; reduces errors on complex, multi-part tasks
C. Recency bias mitigation; to repeat instructions
D. Persona pattern; to change response tone
...
A. `temperature` limits the model to the top X% probability mass of tokens; `top_p` scales overall randomness directly
...
C. Both parameters are functionally identical and always produce the same output
D. `temperature` only affects the first token generated; `top_p` affects all subsequent tokens
...
A. Their vectors will be far apart; measured using Euclidean distance only
...
C. Their vectors will be identical; measured using temperature
D. Embeddings cannot capture semantic similarity, only exact keyword matches
...
A. Grounding means lowering the temperature parameter; it mitigates excessive creativity
...
C. Grounding means fine-tuning the model on labeled data; it mitigates high latency
D. Grounding means caching previous responses; it mitigates token cost
...
B. Keyword search matches meaning; semantic search matches exact terms only
C. Both keyword and semantic search require embeddings
D. Keyword search and vector search are the same technique under different names
...
A. A cached copy of a previous model response; created by the Responses API automatically
...
C. A compressed version of the fine-tuning training dataset; created by LoRA
D. A system message template; created by the persona pattern
...
A. CSV format; the system message should be omitted to reduce file size
...
C. Plain text transcripts; system messages are optional and don't affect accuracy
D. YAML format; only user and assistant messages are needed
...
A. Immediately fine-tune with DPO, since it's the lightest-weight technique
...
C. Skip prompt engineering and RAG entirely, since fine-tuning supersedes both
D. Prepare fewer than 10 training examples to keep costs low
...
B. Creating an entirely new model architecture trained from random initialization
C. Adjusting only the `temperature` and `top_p` inference parameters
D. Building a vector index over the training data without altering the model
...
A. Reducing prompt length only
B. Specific structured output formats (e.g., JSON schema) that few-shot alone can't reliably enforce
C. Enhancing tool-calling accuracy
D. Distillation from a larger model
...
C. Reducing prompt length/token cost
D. Consistent brand style/tone
...
A. Start with fine-tuning, then add RAG, then prompt engineering
B. Start with RAG, then fine-tuning, then prompt engineering
...
D. Always implement all three simultaneously from the start
...
A. RAG optimizes model style/tone; fine-tuning optimizes for context/accuracy
...
C. Both RAG and fine-tuning optimize exclusively for token cost reduction
D. RAG and fine-tuning both require retraining the base model
...
B. Fine-tuning handles domain knowledge, RAG handles style, and prompt engineering handles cost reduction
C. All three strategies perform the identical function of reducing token cost
D. Prompt engineering replaces the need for RAG and fine-tuning once implemented
...
A. Persona pattern alone for both needs
...
C. Fine-tuning alone, since it can encode both the decomposition logic and the case-law data into model weights
D. Zero-shot prompting combined with a higher temperature
...
A. RAG; because it retrieves the persona description at query time
...
C. Increasing `top_p` to reduce token usage
D. The format template pattern; because templates are shorter than personas
...
A. Call `client.chat.completions.create()` with a `previous_response_id` referencing the index
...
C. Call the embedding model directly and return its raw vector as the final answer
D. Skip the `responses.create()` call entirely; Azure AI Search returns chat-ready text automatically
...
A. Skip evaluation and go straight to Reinforcement fine-tuning, since it's always cheapest
...
C. Discard the RAG implementation before fine-tuning, since the two are incompatible
D. Increase `temperature` instead of fine-tuning, since it's cheaper
...
A. Azure AI Search is always cheaper regardless of scale or management overhead
...
C. Foundry IQ cannot be used with the Responses API
D. Azure AI Search only supports keyword search, so it should never be used for agents
...
A. Use RFT only, since it's required for all alignment work
...
C. Use only prompt engineering, since fine-tuning techniques cannot be combined
D. Use SFT exclusively and skip DPO, since combining techniques is unsupported
